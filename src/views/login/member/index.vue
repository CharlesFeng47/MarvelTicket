  ...
  <div class="login-container">
    <el-form autoComplete="on" :model="loginForm" :rules="loginRules" ref="loginForm" label-position="left" label-width="0px"
      ...
      <el-form-item prop="username">
        ...
        <el-input name="username" type="text" v-model="loginForm.username" autoComplete="on" placeholder="请输入会员名" />
      </el-form-item>
      ...
      <el-form-item prop="email" v-if="!wantToLogin">
        <span class="svg-container">
          <svg-icon icon-class="email" />
        </span>
        <el-input name="email" type="text" v-model="loginForm.email" autoComplete="on" placeholder="请输入邮箱" />
      </el-form-item>
      <el-form-item v-show="wantToLogin">
        <el-button type="primary" style="width:100%;" :loading="loading" @click.native.prevent="handleLogin">
          登录
        </el-button>
      </el-form-item>
      <el-form-item v-show="wantToLogin">
        <el-button type="danger" style="width:100%;" @click.native.prevent="changeWantToLoginState">
          去注册
        </el-button>
      </el-form-item>
      <el-form-item v-show="!wantToLogin">
    ...
    methods: {
      ...
      handleSignUp() {
        this.$refs.loginForm.validate(valid => {
          if (valid) {
            this.loading = true
            this.$store.dispatch('MemberSignUp', this.loginForm).then(() => {
              this.loading = false
              this.$router.push({ path: '/' })
            }).catch(() => {
              this.loading = false
